    ...
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({
            error: "Missing required fields for creating a meeting event.",
          });
      }
      ...
      if (!isEnrolled || isEnrolled.status !== "active") {
        return res
          ...
          .json({
            error:
              "Access denied. You must be actively enrolled in this course to host a meeting.",
          });
      }
      ...
      return res
        .status(500)
        .json({ error: "Internal server error while creating meeting." });
    }
  },
);
...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch meeting details." });
    }
  },
);
    ...
    try {
      ...
      if (!availableSlots || typeof availableSlots !== "object") {
        return res
          ...
          .json({
            error:
              "Invalid payload. availableSlots must be a structured JSON object.",
          });
      }
      ...
      if (meeting.status !== "open") {
        return res
          .status(400)
          .json({ error: "This meeting poll is already closed or finalized." });
      }
      ...
      if (!isEnrolled || isEnrolled.status !== "active") {
        return res
          .status(403)
          .json({
            error: "Access denied. You cannot participate in this meeting.",
          });
      }
      ...
      return res
        .status(200)
        .json({ message: "Your availability has been recorded successfully." });
    } catch (error) {
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (!isEnrolled || isEnrolled.status !== "active") {
        return res
          .status(403)
          .json({ error: "Access denied to this meeting heatmap." });
      }
      ...
      const heatmap: Record<
        string,
        Record<string, { count: number; users: { id: number; name: string }[] }>
      > = {};
      ...
      return res
        .status(500)
        .json({ error: "Failed to compute schedule heatmap matrix." });
    }
  },
);
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (!finalizedStartTime || !finalizedEndTime) {
        return res
          .status(400)
          .json({ error: "Missing finalized start or end time bounds." });
      }
      ...
      if (meeting.creatorId !== userId) {
        return res
          ...
          .json({
            error:
              "Access denied. Only the meeting host can finalize the schedule.",
          });
      }
      ...
      if (targetUserIds.length > 0) {
        ...
        const [preferences, targetUsers] = await prisma.$transaction([
          prisma.notificationPreference.findMany({
            where: {
              ...
              OR: [{ courseId: meeting.courseId }, { courseId: null }],
            },
          }),
        ...
        const wantsMeetingNotification = (targetUserId: number) => {
          ...
          const coursePref = preferences.find(
            (p) => p.userId === targetUserId && p.courseId === meeting.courseId,
          );
          ...
          const globalPref = preferences.find(
            (p) => p.userId === targetUserId && p.courseId === null,
          );
        ...
        const notificationsData = targetUserIds
          ...
          .map((targetUserId) => {
            ...
            const targetPref = preferences.find(
              (p) =>
                p.userId === targetUserId &&
                (p.courseId === meeting.courseId || p.courseId === null),
            );
      ...
      return res
        .status(500)
        .json({ error: "Failed to finalize meeting schedule." });
    }
  },
);
