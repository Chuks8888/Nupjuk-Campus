...
async function verifyAccessAndGetBoard(userId: number, courseId: number) {
  ...
  if (!enrollment || enrollment.status !== "active") {
    return {
      error: "Access denied. You must be actively enrolled in this course.",
      status: 403,
    };
  }
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (isNaN(courseId) || page < 1 || limit < 1) {
        return res
          .status(400)
          .json({ error: "Invalid course ID or pagination parameters." });
      }
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (!postExists) {
        return res
          .status(404)
          .json({ error: "Post not found in this course board." });
      }
      ...
      if (postExists.authorId !== userId) {
        ...
        if (shouldNotify) {
          ...
          if (io) {
            io.to(`user_${postExists.authorId}`).emit(
              "new_notification",
              createdNotification,
            );
          }
          ...
          if (emailEnabled) {
            ...
            if (author && author.kaistEmail) {
              ...
              console.log(
                `[Email] Sent to ${author.kaistEmail} about new comment.`,
              );
            }
          }
        }
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (!title || !body) {
        return res
          .status(400)
          .json({ error: "Title and body are required for updating." });
      }
      ...
      if (existingPost.authorId !== userId) {
        return res
          .status(403)
          .json({ error: "Access denied. You can only edit your own posts." });
      }
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (!body) {
        return res
          .status(400)
          .json({ error: "Comment body is required for updating." });
      }
