    ...
    try {
      ...
      const [notifications, totalCount, unreadCount] =
        await prisma.$transaction([
          ...
          }), // Always get total unread badge count
        ]);
...
router.patch(
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      return res
        .status(200)
        .json({ message: `${result.count} notifications marked as read.` });
    } catch (error) {
    ...
    try {
      ...
      if (existingPref) {
        ...
        savedPref = await prisma.notificationPreference.update({
          ...
          data: {
            postCommentEnabled:
              postCommentEnabled ?? existingPref.postCommentEnabled,
            ...
            deadlineReminderTiming:
              deadlineReminderTiming ?? existingPref.deadlineReminderTiming,
          },
        });
      } else {
        // Create new setting
      ...
      return res
        .status(200)
        .json({ message: "Preferences updated.", preferences: savedPref });
    } catch (error) {
