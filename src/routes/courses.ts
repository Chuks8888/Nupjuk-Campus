...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      ...
      // Keeping the stricter security check from the old code
      ...
      // Keeping the crucial 'include: { course }' from the old code
...
// ==========================================
router.post(
  "/:courseId/assignments/:assignmentId/status",
  authenticateToken,
  async (req: AuthRequest, res: Response): Promise<any> => {
    try {
      const userId = req.user!.userId;
      ...
      if (!assignment) {
        return res
          .status(404)
          .json({ error: "Assignment not found in this course." });
      }
